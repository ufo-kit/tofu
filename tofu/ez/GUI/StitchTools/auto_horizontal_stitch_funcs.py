...
class AutoHorizontalStitchFunctions:
    ...
    def write_yaml_params(self):
        try:
            ...
            file_path = os.path.join(
                self.parameters["output_dir"], "auto_vertical_stitch_parameters.yaml"
            )
    ...
    def find_center_parallel_proc(self, index):
        ...
        try:
            ...
            if num_images % 8 == 0:
                ...
                one_thirty_five_degree_image_path = os.path.join(
                    tomo_path, one_thirty_five_degree_image_name
                )
                one_eighty_degree_image_path = os.path.join(tomo_path, one_eighty_degree_image_name)
                two_twenty_five_degree_image_path = os.path.join(
                    tomo_path, two_twenty_five_degree_image_name
                )
                two_seventy_degree_image_path = os.path.join(
                    tomo_path, two_seventy_degree_image_name
                )
                three_fifteen_degree_image_path = os.path.join(
                    tomo_path, three_fifteen_degree_image_name
                )
                three_sixty_degree_image_path = os.path.join(
                    tomo_path, three_sixty_degree_image_name
                )
                ...
                axis_list = [
                    ...
                    self.compute_center(
                        one_thirty_five_degree_image_path, three_fifteen_degree_image_path
                    ),
                    self.compute_center(
                        one_eighty_degree_image_path, three_sixty_degree_image_path
                    ),
                    self.compute_center(
                    ...
                    self.compute_center(
                        three_fifteen_degree_image_path, one_thirty_five_degree_image_path
                    ),
                ]
    ...
    def write_to_log_file(self):
        ...
        try:
            ...
            file_handle.write(
                "Using common set of flats and darks: "
                + str(self.parameters["common_flats_darks"])
                + "\n"
            )
    ...
    def stitch_180_pairs(self, rotation_axis, in_path, out_path, type_str):
        ...
        for index in range(midpoint):
            ...
            output_image_path = os.path.join(
                out_path, type_str, type_str + "_stitched_{:>04}.tif".format(index)
            )
            crop_amount = abs(self.greatest_axis_value - round(rotation_axis))
            self.open_images_stitch_write(
                rotation_axis, crop_amount, first_path, second_path, output_image_path
            )
    ...
    def print_parameters(self):
        ...
        print()
        print(
            "**************************** Running Auto Horizontal Stitch"
            " ****************************"
        )
        print("======================== Parameters ========================")
    ...
    def stitch(self, first, second, axis, crop):
        ...
        second = np.clip(second * k, np.iinfo(np.uint16).min, np.iinfo(np.uint16).max).astype(
            np.uint16
        )
