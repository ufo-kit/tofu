...
class AutoHorizontalStitchGUI(QGroupBox):
    ...
    def set_sample_on_right_checkbox(self):
        LOG.debug(
            "Sample is on right side of the image: "
            + str(self.sample_on_right_checkbox.isChecked())
        )
    ...
    def help_button_pressed(self):
        ...
        h = (
            "Auto-Stitch is used to automatically find the axis of rotation"
            " in order to stitch pairs of images gathered in half-acquisition mode.\n\n"
        )
        h += (
            "The input directory must contain at least one directory named 'tomo' containing .tiff"
            " image files.\n\n"
        )
        h += (
            "The output directory, containing the stitched images,"
            " maintains the structure of the directory tree rooted at the input directory.\n\n"
        )
        h += (
            "If the experiment uses one set of flats/darks the 'Use Common Set of Flats and Darks'"
            " checkbox must be selected. These will then be copied and stitched according to the"
            " axis of rotation of each z-view.\n\n"
        )
        h += (
            "If each z-view contains its own set of flats/darks then"
            " auto_stitch will automatically use these for flat-field correction and stitching.\n\n"
        )
        h += (
            "In most cases of half-acquisition the sample is positioned on the left-side of the"
            " image. Select the 'Is sample on the right side of the image?' checkbox if it is on"
            " the right.\n\n"
        )
        h += (
            "The 'Overlapping Pixels' entry determines the region of"
        ...
        h += (
            "If the dry run button is selected the program will find the axis values without"
    ...
    def delete_button_pressed(self):
        ...
        delete_dialog = QMessageBox.question(
            self,
            "Quit",
            "Are you sure you want to delete the output directory?",
