    ...
    if args.RR:
        ...
        if args.RR_ufo:
            if args.RR_ufo_1d:
                ...
                cmds.append(
                    Ufo.get_filter2d_sinos_cmd(
                        args.tmpdir, args.RR_sig_hor, args.RR_sig_ver, nviews, WH[1]
                    )
                )
        else:
            ...
            if os.path.isfile(path_to_filt):
                ...
                cmdtmp = "python {} --sinos {} --mws {} --mws2 {} --snr {} --sort_only {}".format(
                    path_to_filt,
                    tmp,
                    args.RR_srp_wind_sort,
                    args.RR_srp_wide_wind,
                    args.RR_srp_wide_snr,
                    int(not args.RR_srp_wide),
                )
                cmds.append(cmdtmp)
            else:
...
def main_tk(args, fdt_names):
    ...
    if args.gray256:
        if args.hmin > args.hmax:
            raise ValueError(
                "hmin must be smaller than hmax to convert to 8bit without contrast inversion"
            )
    """
    if args.gray256:
        if args.hmin >= args.hmax:
    ...
    for i, ctset in enumerate(W):
        ...
        if not already_recd(ctset[0], lvl0, recd_sets):
            ...
            if args.ax != 4:
                ...
                if args.ax == 1:
                    ax = FindCOR.find_axis_corr(
                        ctset, args.vcrop, args.y, args.yheight, multipage, args
                    )
            ...
            nviews, WH = frmt_ufo_cmds(
                cmds, ctset, out_pattern, ax, args, Tofu, Ufo, FindCOR, nviews, WH
            )
